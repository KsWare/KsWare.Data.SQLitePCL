	...
	public class ConcurrencyTests {
						...
						while (true) {
							...
							using (var dbConnection = new DbConnection(flags)) {
								...
								System.Diagnostics.Debug.WriteLine(
									$"{Environment.CurrentManagedThreadId} Read records: {records.Count}");
							}
					...
					try {
						while (true) {
							using (var dbConnection =
							       new DbConnection(SQLiteOpenFlags.FullMutex | SQLiteOpenFlags.ReadWrite)) {
								System.Diagnostics.Debug.WriteLine(
									$"{Environment.CurrentManagedThreadId} Start insert");
								...
								System.Diagnostics.Debug.WriteLine(
									$"{Environment.CurrentManagedThreadId} Inserted records");
							}
		...
		public void TestInsertCommandCreation() {
			using (var dbConnection =
				...
				var taskA = Task.Run(() => { dbConnection.Insert(obj1); });
				var taskB = Task.Run(() => { dbConnection.Insert(obj2); });
