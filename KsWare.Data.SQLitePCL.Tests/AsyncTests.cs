...
namespace SQLite.Tests {
	...
	public class Customer {
		...
		[MaxLength(64), Indexed] public string Email { get; set; }

		[MaxLength(64), Indexed] public string Address { get; set; }

		[MaxLength(64), Indexed] public string Country { get; set; }
	...
	public class AsyncTests {
		...
		public class TestEnvironment : IDisposable {
			...
			public TestEnvironment(
				[System.Runtime.CompilerServices.CallerMemberName] string TestMethodName = "unknown") {
...
#if NETFX_CORE
				DatabaseFilePath =
 Path.Combine (Windows.Storage.ApplicationData.Current.LocalFolder.Path, DatabaseName);
				...
				for (var i = 0; i < n; i++) {
					...
					tasks.Add(Task.Run(async () => {
						try {
							...
							var query = await (from c in conn.Table<Customer>() where c.Id == obj.Id select c)
		...
		public void TestExecuteScalar() {
			using (var env = new TestEnvironment()) {
				...
				// check...
				var nodatatask =
					conn.ExecuteScalarAsync<int>("select Max(Id) from customer where FirstName='hfiueyf8374fhi'");
				task.Wait();
