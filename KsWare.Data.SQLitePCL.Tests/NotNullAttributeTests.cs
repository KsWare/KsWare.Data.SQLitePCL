...
namespace SQLite.Tests {
	...
	public class NotNullAttributeTests {
		...
		private class NotNullNoPK {
			...
			[PrimaryKey, AutoIncrement] public int? objectId { get; set; }
			[NotNull] public int? RequiredIntProp { get; set; }
			public int? OptionalIntProp { get; set; }
			[NotNull] public string RequiredStringProp { get; set; }
			public string OptionalStringProp { get; set; }
			[NotNull] public string AnotherRequiredStringProp { get; set; }
		...
		private IEnumerable<SQLiteConnection.ColumnInfo> GetExpectedColumnInfos(Type type) {
#if !NETFX_CORE
			var expectedValues =
				from prop in type.GetProperties(BindingFlags.Public | BindingFlags.Instance | BindingFlags.SetProperty)
				select new SQLiteConnection.ColumnInfo {
					Name = prop.Name,
					notnull = ((prop.GetCustomAttributes(typeof(NotNullAttribute), true).Count() == 0) &&
					           (prop.GetCustomAttributes(typeof(PrimaryKeyAttribute), true).Count() == 0))
						? 0
						: 1
				};
#else
			var expectedValues = from prop in type.GetRuntimeProperties ()
								 select new SQLiteConnection.ColumnInfo {
									 Name = prop.Name,
									 notnull =
 ((prop.GetCustomAttribute<NotNullAttribute> (true) == null) && (prop.GetCustomAttribute<PrimaryKeyAttribute> (true) == null)) ? 0 : 1
								 };
#endif
		...
		public void PrimaryKeyHasNotNullConstraint() {
			using (TestDb db = new TestDb()) {
				...
				Assert.IsTrue(joined.Count() == 0, string.Format(
					"not null constraint was not created for the following properties: {0}"
		...
		public void InsertWithNullsThrowsException() {
			...
			Assert.Fail(
				"Expected an exception of type NotNullConstraintViolationException to be thrown. No exception was thrown.");
		}
